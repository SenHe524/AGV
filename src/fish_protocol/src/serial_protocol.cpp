...
void SerialProtocol::_initSerialProtocol() {
    ...
    flush_serial_port(serial_port_, flush_both,ec);
    _asyncReadSomeData();
...
void SerialProtocol::_recvDataCallback(const boost::system::error_code& error,
    ...
    if (bytes_transferred > 0) {
        ...
        if(isfirst)
        {
            memset(recv_data_buffer_, 0, bytes_transferred);
            bytes_transferred = 0;
            isfirst = false;
            boost::system::error_code ec;
            flush_serial_port(serial_port_, flush_receive,ec);
            _asyncReadSomeData();
            return ;
        }
...
// int SerialProtocol::ProtocolDestory(void) {
//     io_service_.stop();
//     serial_port_.cancel();
//     serial_port_.close();
//     return 0;
